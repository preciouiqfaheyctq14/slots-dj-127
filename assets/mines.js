...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-17c613fa";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-17c613fa">'+'<span class="tile-face-17c613fa tile-front-17c613fa"></span>'+'<span class="tile-face-17c613fa tile-back-17c613fa"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-17c613fa")){return;}
var back=tile.querySelector(".tile-back-17c613fa");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-17c613fa");if(isMine){tile.classList.add("boom-17c613fa");}
if(dim){tile.classList.add("dimmed-17c613fa");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-17c613fa")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-17c613fa");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-17c613fa")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-17c613fa");void panelEl.offsetWidth;panelEl.classList.add("jolt-17c613fa");}
endRound();}
